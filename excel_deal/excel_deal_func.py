...
@GetRunTime
def write_all_duty_record_table(year, month, excel_obj):
    """
    当月员工考勤记录总表
    :return:
    """
    ...
    for j in [0, 1, 2]:
        ...
        for i in xrange(1, table.nrows):
            ...
            if duty_status == '':  # 当天全勤
                ...
                if '上午' in duty_status:
                    pm_duty_content = '8'
                if '下午' in duty_status:
                    am_duty_content = '8'
        ...
        if i != '管理序列':
            ...
            for o in all_index:
                ...
                for p in xrange(0, len(leader_index)):
                    ...
                    b = io.BytesIO()
                    work_book.save(b)  # 存入缓存
                    z.writestr('%s年%s月管理序列%s考勤记录表.xls' % (year, month, name), b.getvalue())
    # 项目合作处理
    # 按 公司拆分
    table1 = original_data_excel.sheet_by_index(1)  # 通过索引顺序获取
